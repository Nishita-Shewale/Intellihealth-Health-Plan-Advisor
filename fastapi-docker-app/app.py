...
from rules import apply_selected_rules, clean_value, get_plan_distribution,get_plans_by_type_from_neo4j
from snowflake_utils import get_snowflake_connection,get_snowflake_connection, normalize_snowflake_data
import re
...
def filter_plans_endpoint(patient_id: PatientID, db: Session = Depends(get_db)):
    ...
    plans, total_count = filter_plans(db, patient_data)
    if not plans:
...
def process_plans(patient_id: PatientID, db: Session = Depends(get_db)):
    ...
    with neo4j_driver.session() as session:
        session.run(
            ...
            is_married=patient_data["is_married"],
                ...
                # Before iterating over plans
        print("\n🔍 Plans Received in process_plans:")
        for idx, plan in enumerate(plans):
            print(f"Plan {idx}: Type -> {type(plan)} | Value -> {plan}")
        ...
        for plan in plans:
            try:
                ...
                session.run(
                    """
                    MERGE (plan:Plan {id: $id})
                    SET plan += $plan_data
                    MERGE (p:Patient {id: $patient_id})
                    MERGE (p)-[:CONSIDERS]->(plan)
                    """,
                    id=getattr(plan, 'id', None) if isinstance(plan, InsurancePlan) else plan.get('id'),
                    plan_data=plan_data,
    ...
    # Apply age-based rule in Neo4j
    preferred_plans = apply_selected_rules(neo4j_driver, patient_data)


    # Check if any preferred plans were found
    if not preferred_plans:
        raise HTTPException(status_code=404, detail="No preferred plans found for the patient in Neo4j")

    first_rule = next(iter(preferred_plans.values()))  # Get the first rule match
    patient_info = first_rule["patient"]  # Patient data is the same across rules

    # Aggregate plans across all rules
    plans_for_patient = [plan for rule in preferred_plans.values() for plan in rule["plans"]]

    print(f"Total plans selected: {len(plans_for_patient)}")

    
    # Return the count and top 10 preferred plans
    return {
    "patient_id": patient_info,
    "preferred_plans_count": len(plans_for_patient),
    "preferred_plans": plans_for_patient[:10], 
    }
...
def plan_distribution(patient_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        plan_distribution, plan_type_distribution = get_plan_distribution(neo4j_driver, patient_id)
        highest_rule_count = max(plan_distribution.keys()) if plan_distribution else 0
...
def get_plans_by_type(patient_id: int, plan_type: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Fetch the plan distribution and plan type distribution from Neo4j for the given patient
        plan_distribution, plan_type_distribution = get_plan_distribution(neo4j_driver, patient_id)

        # Identify the most satisfied rule count
